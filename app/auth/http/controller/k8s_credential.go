...
func (K8sCredential) Token(ctx *gin.Context) {
	...
	aud := panelauth.GetAudience(p)
	token, expiresAt, err := credential.IssueForPrincipalWithAudiences(ctx.Request.Context(), p.Username, p.PermissionName, 10*time.Minute, aud)
	if err != nil {
