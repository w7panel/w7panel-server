...
k3kcli cluster create --storage-class-name=${STORAGE_CLASS_NAME} --mode ${K3K_MODE} --namespace ${K3K_NAMESPACE} --kubeconfig-server=${KUBECONFIG_SERVER} --policy=${K3K_POLICY}  --storage-request-size=$K3K_STORAGE_REQUEST_SIZE --server-args='--kubelet-arg=$cgroup_root' --server-args="--disable=traefik" --server-args="--embedded-registry" --server-args="--disable-network-policy" --server-args="--etcd-arg=quota-backend-bytes=8388608000" ${K3K_NAME} 
# kubectl apply -f - <<EOF
